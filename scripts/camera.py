...
pub = rospy.Publisher('VisionState', String, queue_size=10)  #Float32MultiArray
rospy.init_node('CameraPublisherNode', anonymous=True)
...
def Camera():
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):	
        ...
        rospy.loginfo(str(error+ang))
        pub.publish(str(error+ang))
        rate.sleep()#needs tunning 
        ...
        if key == ord("q") or not rospy.is_shutdown():
